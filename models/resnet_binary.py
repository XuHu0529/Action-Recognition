...
def Binaryconv3x3(in_planes, out_planes, stride=1):
    ...
    return BinarizeConv3d(in_planes, out_planes, kernel_size=(3, 3, 3), stride=stride,
...
def init_model(model):
    for m in model.modules():
        if isinstance(m, BinarizeConv3d):
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1 = BinarizeConv3d(inplanes, planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = BinarizeConv3d(planes, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv3 = BinarizeConv3d(planes, planes * 4, kernel_size=1, bias=False)
    ...
    def _make_layer(self, block, planes, blocks, stride=1,do_bntan=True):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                BinarizeConv3d(self.inplanes, planes * block.expansion,
...
class ResNet_imagenet(ResNet):
    ...
    def __init__(self, num_classes=1000,
        ...
        self.inplanes = 64
        self.conv1 = BinarizeConv3d(3, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
...
class ResNet_cifar10(ResNet):
    ...
    def __init__(self, num_classes=10,
        ...
        n = int((depth - 2) / 6)
        self.conv1 = BinarizeConv3d(3, 16*self.inflate, kernel_size=3, stride=1, padding=1,
                               bias=False)
...
if __name__ == '__main__':
    input = torch.rand((4, 3, 16, 32, 32))
    # model = ResNet_cifar10()
    model= BinarizeConv3d(3, 64, kernel_size=(3, 3, 3), stride=(2, 1, 1),
                     padding=(1, 1, 1), bias=False)
    output = model(input)
